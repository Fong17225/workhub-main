...
//    Đây là service check hạn của gói dịch vụ và chuyển sang lịch sử
//    @PostConstruct
    ...
    public void moveExpiredPackagesToHistory() {
        ...
        if (expiredPackages.isEmpty()) {
            ...
            for (UserPackage userPackage : expiredPackages) {
                ...
                if (servicePackage.getRenewalPolicy() == ServicePackage.RenewalPolicy.renewable) {
                    // Update status to expired
                    userPackage.setStatus(UserPackage.Status.expired);
                    userPackageRepository.save(userPackage);

                    // Update UserBenefits: set cvLimit, jobPostLimit = 0, postAt = none
                    List<UserBenefits> benefitsList = userBenefitRepository.findAllByUserPackageId(userPackage.getId().longValue());
                    for (UserBenefits benefits : benefitsList) {
                        benefits.setCvLimit(0);
                        benefits.setJobPostLimit(0);
                        benefits.setPostAt(UserBenefits.PostAt.none);
                        userBenefitRepository.save(benefits);
                    }
                } else {
                    // Move non_renewable package to history and delete
